...
public class ResultHelper {
    ...
    public static <T extends BaseResult> T failResult(T res, BusinessStatus status) {
    ...
    public static <T extends BaseResult> T sucResult(T res, BusinessStatus status) {
        res.setSuccess(true);
        res.setCode(status.getCode());
        res.setMsg(status.getMessage());
        return res;
    ...
    public static void assertSucResult(BaseResult result) throws BusinessException {
        if (result != null && !result.isSuccess()) {
            throw new BusinessException(result.getCode(), result.getMsg());
        }
    }

    public static void assertSucResult(BaseResult result, BusinessStatus status) throws BusinessException {
        if (result != null && !result.isSuccess()) {
            throw new BusinessException(status);
        }
    }

    public static void assertSucResult(BaseResult result, String msg) throws BusinessException {
        if (result != null && !result.isSuccess()) {
