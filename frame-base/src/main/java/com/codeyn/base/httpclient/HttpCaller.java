...
public class HttpCaller {
    ...
    public Response call(Request request, ProxyInfo proxyInfo) {
        HttpClient httpClient = getHttpClient(request.getConnectionTimeout(), request.getSoTimeout(), proxyInfo);
     ...
     * @return
     */
    public HttpClient getHttpClient(int connectionTimeout, int soTimeout, ProxyInfo proxyInfo) {
    ...
    private HttpGet buildHttpGet(Request request) {
        ...
        if (request.getCharset() != null) {
            try {
                charset = Charset.forName(request.getCharset());
            } catch (Exception e) {
                logger.warn("no support CharSet: {}, use UTF-8 ", request.getCharset());
            }
        }
        try {
            URIBuilder builder = null;
            if (request.getGateway() != null) {
                builder = new URIBuilder(request.getGateway(), charset);
            } else {
                builder = new URIBuilder(charset);
